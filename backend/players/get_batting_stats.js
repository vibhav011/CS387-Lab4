function get_batting_stats(db_client, request, response) { // request and response are parts of the http get method
    ...
    (select (select count(distinct match_id) as matches from ball_by_ball where striker = $1 or non_striker = $1), coalesce(sum(runs_scored), 0) as runs, \
    count(fours)*4 as runs_in_fours, count(sixes)*6 as runs_in_sixes, coalesce(round(sum(runs_scored)*1.0/coalesce(nullif(count(out_type), 0), 1), 2),0) as average, \
    ...
    (select coalesce(count(fifties),0) as fifties, coalesce(max(match_runs),0) as highest_score from (select sum(runs_scored) as match_runs, case when sum(runs_scored)>=50 and sum(runs_scored)<100 then 1 end fifties \
