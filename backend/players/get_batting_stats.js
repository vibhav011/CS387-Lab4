function get_batting_stats(db_client, request, response) { // request and response are parts of the http get method
    ...
    count(fours)*4 as runs_in_fours, count(sixes)*6 as runs_in_sixes, coalesce(round(sum(runs_scored)*1.0/count(out_type), 2),0) as average, coalesce(round(sum(runs_scored)*100.0/count(*), 2),0) as strike_rate \
    from (select match_id, runs_scored, out_type, case when runs_scored = 4 then 1 end fours, case when runs_scored = 6 then 1 end sixes from ball_by_ball where striker = $1) sq1) t1, \
    (select count(fifties) as fifties, max(match_runs) as highest_score from (select sum(runs_scored) as match_runs, case when sum(runs_scored)>=50 then 1 end fifties \
