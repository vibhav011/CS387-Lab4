function get_venue_basic_info(db_client, request, response) { // request and response are parts of the http get method
    ...
    const query = 'select venue_name, city_name as address, capacity, count(match_id) as total_matches_played, max(total_runs) as highest_total_recorded, min(total_runs) as lowest_total_recorded, max(runs_chased) as highest_score_chased from venue, \
    (select team_1.match_id, team1_runs as runs_chased, team1_runs+team2_runs as total_runs from (select match.match_id, team1, runs as team1_runs from (select player_match.match_id, team_id, sum(runs_scored+extra_runs) as runs from \
