function get_venue_basic_info(db_client, request, response) { // request and response are parts of the http get method
    ...
    const query = 'select t1.venue_name, t1.city_name, t1.country_name, t1.capacity,  highest_total_recorded, lowest_total_recorded, highest_score_chased from \
   ...
   (select venue_name, city_name, country_name, capacity, max(innings1_runs*sq.chased) as highest_score_chased from (select match.match_id, sum(runs_scored+extra_runs) as innings1_runs, \
   case when exists(select match_id from match as m2 where (((m2.match_winner = m2.toss_winner and m2.toss_name = \'field\') \
   or (m2.match_winner != m2.toss_winner and m2.toss_name = \'bat\')) and m2.match_id = match.match_id)) then 1 else 0 end chased from match, \
